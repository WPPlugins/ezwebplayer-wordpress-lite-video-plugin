...
Version: 1.5
Description: This video plug-in to plays .flv, .mp4, and  YouTube.com videos directly in your Wordpress Blog posts.
...
$purl = $surl . '/'.PLUGINDIR.'/'.dirname( plugin_basename(__FILE__) );
$defaultvalues = array();
...
//   exit();
    ...
    foreach($arguments as $arg){
        ...
        $carg['PLAYARROW'] = (isset($arg['PLAYARROW'])) ? 'false' : 'true';
...
//    exit();
    ...
    <object align=\"middle\" width=\"%WIDTH%\" height=\"%HEIGHT%\" id=\"%EZWEBPLAYERID%\">
        ...
        <param value=\"%PLAYARROW%\" name=\"TurnPlayButtonOverlayOff\">
        ...
        <embed  width=\"%WIDTH%\"
                ...
                menu=\"false\"
                flashvars=\"TurnPlayButtonOverlayOff=%PLAYARROW%&Skinxml=%SKINURL%&BasicVideoswf={$purl}/Player/BasicVideoSettings/BasicVideoSettings.swf&Videourl=%VIDEOURL%%THUMBNAILURL%\"
                name=\"%EZWEBPLAYERID%\">
...
function ezwebplayerlite_options_page () {        
    ...
    echo '<code>';
    print_r(get_option('ezwplayer_allowplayerarrow'));
...
<div class="wrap" >
    <div style="float:left;margin:15px 0px"><img title="EZWebPlayer Lite Wordpress Plugin" src="<?php echo get_option('home') . '/' . PLUGINDIR . '/' . dirname( plugin_basename(__FILE__) ); ?>/Player/Images/logo.png" /><br /></div>
        ...
        <tbody>
            <tr>
                <td>
                    ...
                    <img src="<?php echo get_option('home') .'/'. PLUGINDIR . '/' . dirname( plugin_basename(__FILE__) ); ?>/Player/Images/thumb_lite.png" alt="" />
                </td>
            </tr>
        </tbody>
    </table>
...
<div class="content">
    <div class="fleft filter_block padl_30">
        <form id="filter" action="<?php echo get_option("home") . "/wp-admin/admin.php?page=". dirname( plugin_basename(__FILE__)) ."/ezwebplayerlite.php" ?>" method="get">
            <input name="page" value="<?php echo dirname( plugin_basename(__FILE__)); ?>/ezwebplayerlite.php" type="hidden" />
            ...
            <ul>
                ...
                <li>
                    <input type="button" value="Clear" onclick='window.location.href = "<?php echo get_option("home") . "/wp-admin/admin.php?page=".dirname( plugin_basename(__FILE__) )."/ezwebplayerlite.php&action=recent" ?>"' />
                </li>
            </ul>
        </form>
    </div>
...
function ezwebplayerlite_media_buttons_context($fl) {
	...
	$image_src = get_option('home') . "/". PLUGINDIR . "/". dirname( plugin_basename(__FILE__) ) ."/playicon.png";
	$fl .= '<a href="' . get_option("home") . '/'. PLUGINDIR . '/' .dirname( plugin_basename(__FILE__)) . '/ezwplite_form.phpTB_iframe=true" class="thickbox" title="' . $media_title . '" onclick="return false;"><img src="' . $image_src . '" title="' . $media_title . '" alt="' . $media_title . '" /></a>';
    return $fl;
}
